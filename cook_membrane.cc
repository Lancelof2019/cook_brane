  ...
  try
    {
	  ...
	  //double *mu_vec_ptr=new double[solid_3d.triangulation.n_active_cells()];
	  double *mu_vec_ptr=mu_vec.data();
	  //std::cout<<solid_3d.triangulation.n_active_cells()<<std::endl;
	  /*std::cout<<"%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"<<std::endl;
	  for(int i=0;i<solid_3d.triangulation.n_active_cells();i++){
                  mu_vec_ptr[i]=1000000;
		  std::cout<<mu_vec_ptr[i]<<std::endl;
	  }
	  std::cout<<"%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%"<<std::endl;
         */
	  ...
	  std:: cout<<"+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"<<std::endl;
	  //std::cout<<"(0,0)"<<solid_3d.tangent_matrix(0,0)<<std::endl;
	  //std::cout<<"(63564,63564)"<<solid_3d.tangent_matrix(solid_3d.tangent_matrix.m()-1,solid_3d.tangent_matrix.n()-1)<<std::endl;

	  std::cout<<"rows"<<solid_3d.tangent_matrix.m()<<std::endl;
	  std::cout<<"cols"<<solid_3d.tangent_matrix.n()<<std::endl;
	 // BlockSparseMatrix<double> copy_global_stiff;
...
/*
        Eigen::MatrixXd eigen_tangent_matrix(solid_3d.tangent_matrix.m(), solid_3d.tangent_matrix.n());
       for (unsigned int i = 0; i < solid_3d.tangent_matrix.m(); ++i) {
          for (unsigned int j = 0; j < solid_3d.tangent_matrix.n(); ++j) {
            eigen_tangent_matrix(i, j) = solid_3d.tangent_matrix(i, j);
         }
       }
...
*/
     ...
     //double det = eigen_tangent_matrix.determinant();
...
//std::cout<<"+++++++++++++++++"<<std::endl;
std::cout<<cell_index_solid<<std::endl;




    //FEValues<dim> fe_values_solid(solid_3d.fe, solid_3d.dof_handler_ref.get_fe().get_update_flags());
   ...
   /*
    for (auto cell = solid_3d.dof_handler_ref.begin_active(); cell != solid_3d.dof_handler_ref.end(); ++cell) {
      if (cell->is_locally_owned()) {
          fe_values_solid.reinit(cell);
          unsigned int dofs_per_cell_tmp = cell->get_fe().dofs_per_cell;
	  //Eigen::VectorXd gradient(dofs_per_cell_tmp);
	  Eigen::VectorXd gradient(1);
	  std::vector<dealii::types::global_dof_index> local_dof_indices(dofs_per_cell_tmp);
	  cell->get_dof_indices(local_dof_indices);
          
        for (unsigned int i = 0; i < dofs_per_cell_tmp; ++i) {
            // 获取当前自由度的全局索引
            dealii::types::global_dof_index global_index = local_dof_indices[i];

            // 根据全局索引从 solver_gradient 中获取对应的值
            double dof_value = solver_gradient[global_index];

            // 使用 dof_value 计算梯度
            gradient + = dof_value;  // 根据 dof_value 计算梯度
        }
	  cell_gradients_solid[cell_index_solid] = gradient;
          ++cell_index_solid;
      }
    }
    */
...
/*	 for (unsigned int i = 0; i < solid_3d.solution_n.size(); ++i) {
        ...
        Eigen::VectorXd adjoint(solid_3d.solution_n.size());
...
/*
     ...
     int den_size=solid_3d.density_gradients.size();
     std::cout<<"size of vector is:"<<den_size<<std::endl;
     for(int i=0;i<den_size;i++){
       std::cout<<"The ["<<i<<"] element:"<<solid_3d.density_gradients[i]<<std::endl;
...
*/
	...
	// std::cout<<"some value in grad:"<<solid_3d.gradient[25]<<std::endl;
	...
	// std::cout<<"norm of output"<<output_vector.l1_norm()<<std::endl;


	
	 // A_direct.vmult(output_vector,solid_3d.system_rhs);
        // solid_3d.tangent_matrix.vmult(output_vector,solid_3d.system_rhs);
	 
	 //const unsigned int num_blocks = matrix_size / block_size;
	 //const unsigned int matrix_size=get_rows;
	 //const unsigned int num_blocks = matrix_size / block_size;
	 ...
	 /*double 
         for(int i=0;i<solid_3d.tangent_matrix.m();i++){
		 for(int j=bLockvector,double.0;j<solid_3d.tangent_matrix.n();j++){
                 full_tangent_matrix(i,j)=solid_3d.tangent_matrix(i,j);
		 }
	  } 
       */
/*
...
*/
         ...
         //dealii::SparseDirectUMFPACK inverse_solver;

	 //inverse_solver.initialize(solid_3d.tangent_matrix);

        // dealii::BlockSparseMatrix<double> inv_tangent_matrix;
	 //inverse_solver.inverse(solid_3d.tangent_matrix, inv_tangent_matrix);

        // solid_3d.tangent_matrix.invert(inv_tangent_matrix);
         //dealii::FullMatrix<double> inv_tangent_matrix(solid_3d.tangent_matrix.m(),solid_3d.tangent_matrix.n());


	 //solid_3d.tangent_matrix.invert(inv_tangent_matrix);

         //dealii::InverseMatrix<dealii::FullMatrix<double>> inverse_matrix(solid_3d.tangent_matrix);


        // std::cout<<"after norm (0,63564)"<<solid_3d.system_rhs[solid_3d.system_rhs.size()-1]<<std::endl;

	// std::cout<<"after norm (0,2)"<<solid_3d.system_rhs[2]<<std::endl;

       //	BlockSparseMatrix<double> output_vector(get_rows);
